...
/**
 * Load raw history from chunked storage without updating state
 * Used by dashboard and other read-only contexts
...
/**
 * Load history from chunked storage and update state
 * Used by keno-tool content script
 * @returns {Promise<Array>} History array
 */
export function loadHistory() {
  return loadHistoryRaw().then(history => {
    state.currentHistory = history;
    state.recentPlays = getRecentPlays(5);
    stateEvents.emit(EVENTS.HISTORY_UPDATED, state.currentHistory);

    // Migrate old format in background if needed
