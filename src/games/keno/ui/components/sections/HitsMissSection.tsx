// src/ui/components/sections/HitsMissSection.tsx
...
export function HitsMissSection() {
  ...
  const [missesArray, setMissesArray] = useState<number[]>([]);
  const [hitsCollapsed, setHitsCollapsed] = useState<boolean>(false);
  const [missesCollapsed, setMissesCollapsed] = useState<boolean>(true);
  const [roundId, setRoundId] = useState<number>(Date.now());
  const { showLiveStats } = useModals();

  // Subscribe to round updates
  useEffect(() => {
    const updateFromEvent = (eventData: RoundSavedEvent) => {
    ...
    if (state.trackerHits && Array.isArray(state.trackerHits)) {
      setHitsArray(state.trackerHits as number[]);
    }
    if (state.trackerMisses && Array.isArray(state.trackerMisses)) {
      setMissesArray(state.trackerMisses as number[]);
    }
